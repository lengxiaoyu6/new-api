...
func TestProcessChannelErrorPreservesMappedStatusAndDomainWithoutLeakingChannelMetadata(t *testing.T) {
	...
	ctx.Request.Header.Set("X-Forwarded-Host", "api.example.com, internal.example.com")
	ctx.Set("id", 7)
	...
	common.SetContextKey(ctx, constant.ContextKeyChannelStatusCodeResponseMapping, `{"502":{"status_code":503,"message":"Upstream is busy, please retry later."}}`)
	...
	require.NoError(t, err)
	assert.Equal(t, "status_code=503, Upstream is busy, please retry later.", stored.Content)
	assert.Equal(t, float64(http.StatusBadGateway), storedOther["original_status_code"])
	assert.Equal(t, float64(http.StatusServiceUnavailable), storedOther["status_code"])
	assert.Equal(t, "api.example.com", storedOther["request_domain"])
	...
	assert.Equal(t, float64(http.StatusBadGateway), userOther["original_status_code"])
	assert.Equal(t, float64(http.StatusServiceUnavailable), userOther["status_code"])
	assert.Equal(t, "api.example.com", userOther["request_domain"])
	for _, key := range []string{"channel_id", "channel_name", "channel_type"} {
