...
func InitDB() (err error) {
	...
	if err == nil {
		...
		if common.SkipDatabaseMigration {
			common.SysLog("SKIP_DB_MIGRATION set, database migration skipped")
			return nil
		}
...
func InitLogDB() (err error) {
	...
	if err == nil {
		...
		if common.SkipDatabaseMigration {
			common.SysLog("SKIP_DB_MIGRATION set, log database migration skipped")
			return nil
		}
		common.SysLog("database migration started")
		err = migrateLOGDB()
...
` + "`allowed_groups`" + ` varchar(255) DEFAULT '',
...
PRIMARY KEY (` + "`id`" + `)
	...
	required := []sqliteColumnDef{
		...
		{Name: "allowed_groups", DDL: "`allowed_groups` varchar(255) DEFAULT ''"},
