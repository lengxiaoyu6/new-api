...
func TestSharedTaskBillingExpressionSelectionAndFrozenSettlement(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "channel profile retains alias identity", plugin: "billing-beta", model: "alias-model", mapping: `{"alias-model":"declared-model"}`, modelExpr: baseExpr, mode: "tiered_expr", variants: map[string]string{"billing-beta::declared-model": betaExpr}, channelExpr: aliasExpr, wantExpr: aliasExpr},
		{name: "channel profile must match executing plugin schema", plugin: "billing-beta", model: "declared-model", variants: map[string]string{"billing-beta::declared-model": betaExpr}, channelExpr: alphaExpr, wantPriceError: true},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.channelExpr != "" {
				assert.Equal(t, "channel-review", info.TieredBillingSnapshot.ProfileKey)
				assert.Equal(t, billing_setting.BillingProfileSourceChannel, info.TieredBillingSnapshot.ProfileSource)
				assert.Equal(t, 42, info.TieredBillingSnapshot.ChannelID)
			}
			assert.NotEqual(t, "model_price_error", taskErr.Code)
