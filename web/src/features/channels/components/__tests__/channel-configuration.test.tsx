...
test.each([
  {
    ...
    label: 'Channel Billing Profiles',
  },
  {
    kind: 'status response',
    saved: {
      status_code_response_mapping: JSON.stringify({
        '429': { status_code: 503, message: 'Busy' },
      }),
    },
    label: 'Status Code Response Mapping',
  },
])(
  'saved $kind overrides remain visible and survive editing',
  async ({ saved, label }) => {
    Object.assign(editingChannel, saved)
    const put = vi
      .spyOn(api, 'put')
    ...
    for (const [field, value] of Object.entries(saved)) {
      expect(JSON.parse(payload[field])).toMatchObject(JSON.parse(value))
    }
  }
)
