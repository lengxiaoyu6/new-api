...
export function ChannelMutateDrawer({
  ...
  const currentBillingProfiles = formValues.billing_profiles
    ...
    >
      ...
      <div className='space-y-4 rounded-lg border p-4'>
        ...
        <div className='bg-muted/20 flex flex-col gap-3 rounded-md border p-3 sm:flex-row sm:items-center sm:justify-between'>
          ...
          <Button
            ...
            size='sm'
            disabled={
              isSubmitting || sensitiveLocked || !billingProfilesJsonValid
            }
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem className='space-y-3'>
            <div className='space-y-1'>
              <FormLabel>{t('Status Code Response Mapping')}</FormLabel>
              <FormDescription>
                {t('Customize error responses by upstream status code')}
              </FormDescription>
            </div>
            <FormControl>
              <JsonEditor
                ...
                emptyMessage={t('No status code response mappings configured.')}
                template={{
                  '429': {
                    status_code: 503,
                    message: 'Upstream is busy, please retry later.',
                    type: 'server_error',
